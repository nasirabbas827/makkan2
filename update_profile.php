...
$sql = "SELECT username, full_name, email, contact_number, address, profile_pic FROM users WHERE user_id = ?";
if ($stmt = mysqli_prepare($conn, $sql)) {
    mysqli_stmt_bind_param($stmt, "i", $user_id);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_bind_result($stmt, $username, $full_name, $email, $contact_number, $address, $profile_pic);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['profile_pic']) && $_FILES['profile_pic']['error'] == 0) {
        ...
        if (move_uploaded_file($_FILES["profile_pic"]["tmp_name"], $target_file)) {
            // Save only the image name (e.g., name.jpg) in the database
    ...
    if ($email_count > 0) {
        ...
        $sql_update = "UPDATE users SET full_name = ?, email = ?, contact_number = ?, address = ?, profile_pic = ? WHERE user_id = ?";
        if ($stmt = mysqli_prepare($conn, $sql_update)) {
            mysqli_stmt_bind_param($stmt, "sssssi", $full_name, $email, $contact_number, $address, $profile_pic, $user_id);
...
<body>
    ...
    <div>
        ...
        <div>
            ...
            <img src="seller/uploads/<?php echo htmlspecialchars($profile_pic); ?>" alt="Profile Picture" width="100">
        </div>
